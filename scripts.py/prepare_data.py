...
def split_data(processed_data_path, output_dir):
    """
    Reads the processed parquet file and splits it into train, validation, and test sets.
    The split is 80% train, 10% validation, 10% test.
    Saves them as parquet files in the processed data directory.

    Args:
        processed_data_path (str): Path to the input processed_data.parquet file.
    ...
    try:
        ...
        logging.error("Please ensure you have run 'ml/preprocess.py' first.")
        return
    ...
    val_df, test_df = train_test_split(
        ...
        test_size=0.5,
        random_state=42,
        stratify=temp_df['label'] # Stratify again to be sure
    )
    ...
    logging.info(f"Split data files saved to {output_dir}")
