...
def train_transformer(model_key: str):
    ...
    Args:
        model_key (str): The key for the model in config.TRANSFORMER_CONFIG['model_names']
                         (e.g., 'distilbert' or 'albert').
    """
    # --- 1. Setup ---
    ...
    # Use GradScaler for mixed-precision (FP16) training if on CUDA
    ...
    for epoch in range(config.TRANSFORMER_CONFIG['epochs']):
        ...
        for i, batch in enumerate(progress_bar):
            ...
            total_loss += loss.item() * config.TRANSFORMER_CONFIG['gradient_accumulation_steps']
        ...
        logging.info(f"Validation Metrics for Epoch {epoch+1}: {metrics}")
        all_results[f'epoch_{epoch+1}'] = metrics
    ...
    results_filename = f'transformer_{model_key}_results.json'
    save_json_results(all_results, results_filename, config.RESULTS_DIR)
    logging.info(f"--- Training for {model_key} complete. ---")

if __name__ == '__main__':
    # --- Setup Logging ---
    # Configure logging once for the entire script run to capture all model training in one file.
    ...
    # Loop through the models defined in the config and train each one
